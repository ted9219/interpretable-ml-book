...
기계학습 모델을 해석 가능하도록 만드는 한가지 방법은 선형 모델이나 의사결정 나무와 같은 원래 [해석이 가능한 알고리즘](#simple)을 사용하여 모델을 만드는 것입니다.
또 한가지 방법은 모든 지도 학습 모델에 활용가능한 모델 애그노스틱 해석 도구 [model-agnostics interpretation tools](#agnostics) (역자주: 모델 알고리즘 종류에 특이적이지 않고 범용적으로 활용할 수 있는 해석 방법. 불가지론적 모델 해석 도구) 를 사용하는 것입니다. 
애그노스틱 모델 해석 방법 단원에서는 부분 의존도 도식 _partial dependence plots_ 과 순열 특성 중요도 _permutation feature importance_ 와 같은 방법들을 다루게 됩니다. 
애그노스틱 모델 해석 방법들은 모델 입력 _model input_ 을 변화시키고 이에 다른 예측 결과 _prediction output_ 의 변화를 측정하여 계산됩니다.
데이터 인스턴스로 모델의 해석을 표현하는 모델 애그노스틱 해석 방법들은 [예제 기반 해석](#example-based)에서 다루었습니다.
모든 모델 애그노스틱 해석 방법들은 개별 예측에 대하여 설명하는지 혹은 전체 데이터에 대한 모델의 전반적인 행태를 설명하는지에 따라서 더욱 세분화 할 수 있습니다.
다음과 같은 방법들은 모델의 전반적인 행태에 대하여 설명합니다: [Partial Dependence Plots](#pdp), [Accumulated Local Effects](#ale), [Feature Interaction](#interaction), [Feature Importance](#feature-importance), [Global Surrogate Models](#global) and [Prototypes and Criticisms](#proto).
반면, 다음과 같은 방법들은 개별 예측에 대하여 설명합니다다: [Local Surrogate Models](#lime), [Shapley Value Explanations](#shapley), [Counterfactual Explanations](#counterfactual) (and closely related: [Adversarial Examples](#adversarial)). 
일부 방법들은 전반적인 행태와 개별 예측 설명에 모두 활용할 수 있습니다: [Individual Conditional Expectation](#ice) and [Influential Instances](#influential).
이 책은 [해석가능한 기계학습의 미래](#future) 가 어떤 모습일지에 대하여 낙관적인 전망을 가지고 결론지었습니다.
